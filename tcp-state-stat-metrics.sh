...
echo 
echo "Start to collect TCP State Stat metrics, interval: ${INTERVAL}s"

while true; do 

netstat -ntp | grep tcp \
 | awk '$4~/:'${PORT_PATTERN:-80}'$/ {a[$6]++}; END { for(b in a) printf "container_tcp_state_count{host=\"'${HOSTNAME}'\", app=\"'${APP_NAME}'\", job=\"tcp-state-stat\", direction=\"inbound\", state=\"%s\"} %s\n", b, a[b]}' \
 | curl --data-binary @- $METRICS_SERVER_API

netstat -ntp | grep tcp \
 | awk '$4!~/:'${PORT_PATTERN:-80}'$/ {a[$6]++}; END { for(b in a) printf "container_tcp_state_count{host=\"'${HOSTNAME}'\", app=\"'${APP_NAME}'\", job=\"tcp-state-stat\", direction=\"outbound\", state=\"%s\"} %s\n", b, a[b]}' \
 | curl --data-binary @- $METRICS_SERVER_API
